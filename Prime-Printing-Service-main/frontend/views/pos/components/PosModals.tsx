                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="bg-[#f4f5f8] p-6 rounded border border-[#d4d7dc] space-y-3">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-bold text-[#393a3d]">{currency}{pricingState.sellingPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="pt-3 border-t border-[#d4d7dc] flex justify-between items-center">
                            ...
                            <span className="text-xl font-bold text-[#0077c5]">{currency}{(pricingState.sellingPrice * quantity).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                    </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {option.coversPerCopy > 0 && (
                                                    <div className="flex flex-col items-end">
                                                        ...
                                                        <span className="text-[10px] text-[#8d9096]">
                                                            {copies * option.coversPerCopy} total â†’ {formatNumber(materialUnitsNeeded)} unit(s)
                                                        </span>
                                                    </div>
                                                )}
                                                {option.coversPerCopy > 0 && (
...
export const CustomerModal: React.FC<{
    ...
    // Build a unified, deduplicated customer list:
    // 1. Start with all named Customer records (have wallet, contact info, etc.)
    // 2. Supplement with any additional names found only in invoices
    const customerList = useMemo(() => {
        const list: { name: string; walletBalance: number; outstandingDebt: number }[] = [];
        const addedNames = new Set<string>();

        // Add all proper Customer records first
        (customers as any[]).forEach((c: any) => {
            const name = c.name || c.customerName;
            if (!name) return;
            const custInvoices = (invoices || []).filter(
                (i: any) => i.customerName === name && i.status !== 'Paid' && i.status !== 'Draft'
            );
            const outstandingDebt = custInvoices.reduce(
                (sum: number, i: any) => sum + (i.totalAmount - (i.paidAmount || 0)), 0
            );
            list.push({
                name,
        ...
        // Supplement with invoice-only customer names (legacy / no Customer record)
        (invoices || []).forEach((inv: any) => {
            ...
            list.push({ name, walletBalance: 0, outstandingDebt });
            addedNames.add(name);
        });
    ...
    return (
        <div className="absolute inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-[2px]">
            <div className="bg-white rounded shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col overflow-hidden border border-[#d4d7dc]">
                ...
                <div className="px-6 py-4 bg-white border-b border-[#d4d7dc] flex justify-between items-center gap-4 shrink-0">
                    {/* Search box */}
                    <div className="relative flex-1">
                        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8d9096]" />
                        <input
                            type="text"
                            ...
                            autoFocus
                            className="w-full pl-8 pr-4 py-2 border border-[#babec5] rounded text-sm focus:border-[#0077c5] outline-none bg-[#f4f5f8]"
                        />
                    </div>
                    <button
                ...
                <div className="p-2 overflow-y-auto flex-1 divide-y divide-[#f4f5f8] custom-scrollbar">
                    {filteredCustomers.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 text-[#8d9096]">
                            <Search size={36} className="mb-3 opacity-20" />
                            <p className="text-sm font-medium">No customers found</p>
                            <p className="text-xs mt-1">Try a different name or add a new customer</p>
                        </div>
                    )}
                    {filteredCustomers.map(({ name, walletBalance, outstandingDebt }) => (
                                ...
                                <div>
                                    ...
                                    {walletBalance > 0 && (
                                        <div className="flex items-center gap-1 mt-0.5">
                                            ...
                                            <span className="text-[10px] text-emerald-600 font-semibold">Wallet: {currency}{walletBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            {(outstandingDebt > 0 || walletBalance > 0) && (
                                <div className="text-right space-y-0.5">
                                    {outstandingDebt > 0 && (
                                        <div>
                                            <div className="text-xs font-bold text-[#d52b1e]">{currency}{outstandingDebt.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                    ...
                    <div key={order.id} className="px-6 py-5 flex justify-between items-center hover:bg-[#f4f5f8] transition-all group">
                        <div className="space-y-1">
                            ...
                            <div className="text-xs text-[#6b6c7f] flex items-center gap-3">
                                <span>{new Date(order.date).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
...
export const QuickPrintModal: React.FC<{
    service: any;
